...
const registerAuxiliar = async (req, res) => {
  ...
  if (!numero_empleado || !contrasena) {
    return res
      .status(400)
      .json({ message: "Número de empleado y contraseña son requeridos" });
  }
  ...
  try {
    ...
    if (face_image) {
      try {
        ...
        if (response.data.faces && response.data.faces.length > 0) {
          ...
          return res
            .status(400)
            .json({ message: "No se detectó rostro en la imagen" });
        }
      } catch (error) {
    ...
    if (err.code === "23505") {
      return res
        .status(400)
        .json({ message: "El número de empleado ya existe" });
    }
...
const login = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: "Error al generar token", error: err.message });
  }
};
